...
class TestGetClientIp(unittest.TestCase):
    ...
    def test_get_client_ip_azure_header(self):
        ...
        with self.app.test_request_context('/', headers={'X-Azure-ClientIP': '203.0.113.1'}):
            ip = get_client_ip()
    ...
    def test_get_client_ip_remote_addr(self):
        ...
        with self.app.test_request_context('/', environ_base={'REMOTE_ADDR': '203.0.113.1'}):
            ip = get_client_ip()
            self.assertEqual(ip, '203.0.113.1')
    
    def test_get_client_ip_with_port(self):
        """Test IP extraction when port is included."""
        with self.app.test_request_context('/', headers={'X-Azure-ClientIP': '203.0.113.1:8080'}):
            ip = get_client_ip()
            self.assertEqual(ip, '203.0.113.1')
